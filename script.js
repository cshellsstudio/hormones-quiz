...
function initQuiz() {
  ...
  dotsWrap.innerHTML = ''; // Clear dots to avoid duplicates
...
function goNext() {
  ...
  // Kung nasa huling tanong na at pinindot ang continue
  if (current >= TOTAL) {
    const data = computeResult();

    // I-save nang ligtas sa localStorage ng browser para tago sa URL bar
    localStorage.setItem('quiz_tags', data.tags.join(','));
    localStorage.setItem('quiz_stage', data.stage);
    localStorage.setItem('quiz_severity', data.severityLabel);
    localStorage.setItem('quiz_score', data.totalScore);
  ...
  // Kung hindi pa huling tanong, magpalit lang ng slide pataas
...
function initResults() {
  ...
  // Babasahin ang tinagong data mula sa localStorage
  const storedTags = localStorage.getItem('quiz_tags');
  const tags       = storedTags ? storedTags.split(',') : [];
  ...
  // Build CTA URL kasama ang quiz data para ma-pre-fill ang GHL Calendar fields
  ...
  // Render content sa screen ng user
