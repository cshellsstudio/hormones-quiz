...
const CONFIG = {
  ...
  webhookUrl: 'https://YOUR_GHL_WEBHOOK_URL',
...
function goNext() {
  ...
  if (current >= TOTAL) {
    ...
    // Also save to sessionStorage for email capture
    sessionStorage.setItem('quizResultForEmail', JSON.stringify({
      tags:     result.tags,
      stage:    result.stage,
      severity: result.severityLabel,
      score:    result.totalScore,
...
function submitEmail() {
  ...
  const raw = sessionStorage.getItem('quizResultForEmail');
  ...
  .then(() => {
    ...
    if (emailCard) emailCard.innerHTML = `
      <div style="text-align:center;padding:8px 0 4px;">
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" style="margin-bottom:10px"><circle cx="12" cy="12" r="10" fill="rgba(74,124,111,0.12)"/><path d="M7 12.5L10.5 16L17 9" stroke="#4a7c6f" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        <p style="font-family:'Cormorant Garamond',serif;font-size:1.2rem;color:var(--text-dark);margin-bottom:6px;">You're all set, ${firstName || 'there'}!</p>
        <p style="font-size:0.85rem;color:var(--text-mid);line-height:1.6;">We've saved your hormone profile. Now choose a time that works for you.</p>
      </div>`;
  ...
  .catch(() => {
    // Still show CTA even if webhook fails — don't block the user
    if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Continue to Booking →'; }
    const ctaSection = document.getElementById('ctaSection');
    if (ctaSection) ctaSection.style.display = 'block';
    if (emailError) { emailError.textContent = 'Something went wrong saving your info, but you can still book below.'; emailError.style.display = 'block'; }
  });
}
